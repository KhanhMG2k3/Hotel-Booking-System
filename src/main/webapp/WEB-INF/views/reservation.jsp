<%@ page contentType="text/html;charset=UTF-8" language="java" %>
  <%@ taglib prefix="c" uri="jakarta.tags.core" %>
    <%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %>
      ...
      <section class="site-hero inner-page overlay hero-reservation" data-stellar-background-ratio="0.5">
        <div class="container">
          ...
          <div class="row">
            <div class="col-md-7" data-aos="fade-up" data-aos-delay="100">
              ...
              <form action="${pageContext.request.contextPath}/reservation" method="post"
                class="bg-white p-md-5 p-4 mb-5 border rounded shadow-sm">
                    ...
                    <select name="room_id" id="room_id" class="form-control" required>
                      <c:forEach var="r" items="${rooms}">
                        <option value="${r.id}" ${r.id==selectedRoomId ? 'selected' : '' }>
                          ${r.roomName} (${r.roomType.typeName}) -
                          <fmt:formatNumber value="${r.pricePerNight}" pattern="#,###" /> VNĐ/đêm
                        </option>
                      </c:forEach>
                    </select>
                  </div>
                ...
                <div class="row">
                  <div class="col-md-6 form-group">
                    ...
                    <input type="text" id="checkin_date" name="checkin_date" class="form-control" autocomplete="off"
                      required placeholder="Chọn ngày đến">
                  </div>
                  <div class="col-md-6 form-group">
                    <label class="text-black font-weight-bold" for="checkout_date">Ngày trả phòng *</label>
                    <input type="text" id="checkout_date" name="checkout_date" class="form-control" autocomplete="off"
                      required placeholder="Chọn ngày đi">
                  </div>
                </div>
                ...
                <div class="row mb-4">
                  <div class="col-md-12 form-group">
                    ...
                    <textarea name="notes" id="notes" class="form-control" cols="30" rows="3"
                      placeholder="Giờ check-in dự kiến, yêu cầu xe đưa đón hoặc các lưu ý đặc biệt..."></textarea>
                  </div>
                </div>

                <div class="row">
                  <div class="col-md-12 form-group">
                    <input type="submit" value="Xác Nhận Đặt Phòng (Thanh Toán Khi Check-in)"
                      class="btn btn-primary btn-block text-white font-weight-bold py-3 shadow">
                  </div>
                </div>
              </form>
            ...
            <div class="col-md-5" data-aos="fade-up" data-aos-delay="200">
              <div class="bg-white p-4 rounded border shadow-sm mb-4">
                <h4 class="h5 font-weight-bold text-primary mb-3"><i class="fa fa-phone-square mr-2"></i> Liên Hệ Hỗ Trợ
                  Trực Tiếp</h4>
                <p class="text-muted small mb-2"><span class="font-weight-bold text-dark">Địa chỉ:</span> 123 Đường Võ
                  Nguyên Giáp, Sơn Trà, Đà Nẵng</p>
                <p class="text-muted small mb-2"><span class="font-weight-bold text-dark">Hotline / Zalo:</span> (+84)
                  090 123 4567</p>
                <p class="text-muted small mb-2"><span class="font-weight-bold text-dark">Email:</span>
                  info@sogohomestay.com</p>
                <p class="text-muted small mb-0"><span class="font-weight-bold text-dark">Thời gian phục vụ:</span> 24/7
                  hàng ngày</p>
              </div>

              <div class="bg-light p-4 rounded border">
                <h5 class="font-weight-bold text-dark mb-2"><i class="fa fa-credit-card text-primary mr-2"></i> Phương
                  Thức Thanh Toán</h5>
                <p class="text-muted small mb-0">Hỗ trợ chuyển khoản ngân hàng, quét mã QR VietQR hoặc thanh toán tiền
                  mặt trực tiếp khi nhận phòng tại quầy lễ tân Homestay.</p>
              </div>
            </div>
          </div>
        </div>
